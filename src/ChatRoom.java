...
public class ChatRoom extends Thread implements ActionListener {
	static JFrame frmChat;//聊天室的主窗口

	JPanel pnlChat;//聊天室的主面板

	JButton btnCls, btnExit, btnSend, btnClear, btnSave;//按钮，清屏、退出、发送、保存

	JLabel lblUserList, lblUserMessage, lblSendMessage, lblChatUser;//标签，用户列表、聊天信息、在线人数

	JLabel lblUserTotal, lblCount, lblBack;

	JTextField txtMessage;//用户输入聊天内容的文本框

	java.awt.List lstUserList;//显示在线用户列表的列表框

	TextArea taUserMessage;//显示聊天信息的文本区域

	JComboBox cmbUser;//选择聊天对象的下拉框

	JCheckBox chPrivateChat;//私聊复选框

	String strServerIp, strLoginName;//服务器IP地址和用户登录名

	Thread thread;//刷新聊天信息的线程

	final JLabel headLabel = new JLabel();//显示用户头像的标签

	Dimension scrnsize;

	Toolkit toolkit = Toolkit.getDefaultToolkit();//用于获取屏幕尺寸等系统信息

	Message messobj = null;//用于与服务器通信的消息对象

	String serverMessage = "";//服务器发送的系统消息

	// 构造方法,初始化聊天室界面，并启动刷新线程
